...
router.get('/api/getUsers', async function(req, res) {
        try {
            ...
            if (req.query.export) {
                ...
                fs.writeFileSync("public/csv/users.csv", csv);
                console.log(csv)
                res.json({ url: "http://localhost:3000/csv/users.csv" });
                return;
            }
            if (req.query.emailUrl || req.query.emailVal) {
                let email = req.query.emailVal;
                let url = "http://localhost:3000/csv/users.csv";
                "use strict";
                async function main() {
