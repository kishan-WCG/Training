...
$(document).ready(function() {
    ...
    function getAllUser(obj) {
        ...
        $.ajax({
            ...
            type: "get",
            // data: btnvalue,
            success: function({ data, totalUsers, url }) {
                if (url) {
                    let exportDiv = $('<a />');
                    exportDiv.attr('download', 'users.csv');
                    exportDiv.attr('href', url);
                    $('body').append(exportDiv);
                    exportDiv[0].click();
                    $('body').remove(exportDiv);
                    return;
                }
                $('.tbl-data').empty()
    ...
    $(document).on("click", ".exportEmailBtn", function() {
        ...
        getAllUser({ emailUrl: val, emailtext: email })
